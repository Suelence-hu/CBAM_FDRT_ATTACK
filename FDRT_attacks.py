...
def show_images_diff(original_img,original_label,adversarial_img,adversarial_label,eps):
    ...
    difference1 = adversarial_img - original_img
    #(-1,1)  -> (0,1)
    difference=difference1 / abs(difference1).max()/2.0+0.5
    plt.imshow(difference,cmap=plt.cm.gray)
    plt.title('Eps={:.5f}'.format(float(abs(difference1[0,0,0]))))
    plt.axis('off')
    plt.tight_layout()
...
atks = [
    ...
    # FDRT_MIFGSM(model, eps=20/255, steps=10, decay=1.0),
    # FDRT_NIFGSM(model, eps=8/255, alpha=2/255, steps=5, decay=1.0),
    # FDRT_PGD(model, eps=8/255, alpha=1/255, steps=40, random_start=True)
