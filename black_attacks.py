...
atks = [
    ...
    # BIM(model, eps=20/255, alpha=2/255, steps=8),
    # MIFGSM(model, eps=20/255, steps=5, decay=1.0),
    # NIFGSM(model, eps=20/255, alpha=2/255, steps=10, decay=1.0),
    # PGD(model, eps=20/255, alpha=1/255, steps=40, random_start=True),

    # FDRT_FGSM(model, eps=20/225),
    # FDRT_BIM(model, eps=20/255, alpha=2/255, steps=8)
    # FDRT_MIFGSM(model, eps=20/255, steps=5, decay=1.0),
    FDRT_NIFGSM(model, eps=20/255, alpha=2/255, steps=10, decay=1.0),
    # FDRT_PGD(model, eps=20/255, alpha=1/255, steps=40, random_start=True)
...
for atk in atks:
    ...
    for images, labels in data_loader:
        ...
        outputs = model_target(adv_images)
        y_adv = (model_target(adv_images)).cpu().detach().numpy()
        ...
        i = i + 1
    print("========================")
    print("迁移攻击成功率为：{}".format(accuracy / i))
